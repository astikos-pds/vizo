...
const effectivePermission = computed({
  get: () => {
    return form.mode !== t('components.assignedUsers.custom') && selectedPermissionPreset.value
...
const hasUnsavedChanges = computed(() => {
  const modeChanged =
    (form.mode === t('components.assignedUsers.custom')) !== (assignedUser.permissionMode === "CUSTOM");
...
const onSubmit = async (event: FormSubmitEvent<AssigneeSchema>) => {
  ...
  const saved = await changeAssigneePermission(
    ...
    assignedUser.id,
    {
      permissionMode: event.data.mode === t('components.assignedUsers.custom') ? "CUSTOM" : "PRESET",
      ...event.data,
    }
  );
